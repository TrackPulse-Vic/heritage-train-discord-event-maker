...
for event in parsed_data:
    ...
    if event['book_link'] != None:
        try:
            asyncio.run(addEvent(SERVER, event['name'], event['description'], event['date'], event["end_date"], {'location': event['book_link']}))
